"use client"
const Moon = ({
  className,
  color,
  fill = "currentColor",
  height,
  strokeColor,
  viewBox = "0 0 24 24",
  width,
  strokeWidth = 0,
  onClick = () => {},
  size = 24,
}: IconProps) => {
  return (
    <svg
      stroke={strokeColor || fill || color}
      fill={fill}
      strokeWidth={strokeWidth?.toString()}
      viewBox={viewBox}
      className={className}
      height={height?.toString() || size?.toLocaleString()}
      width={width?.toString() || size?.toLocaleString()}
      xmlns="http://www.w3.org/2000/svg"
      onClick={onClick}
    >
      <path fill="none" d="M0 0h24v24H0z"></path>
      <path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 01-4.4 2.26 5.403 5.403 0 01-3.14-9.8c-.44-.06-.9-.1-1.36-.1z"></path>
    </svg>
  );
};

const Sun = ({
  className = "",
  color,
  fill = "currentColor",
  height,
  strokeColor,
  viewBox = "0 0 24 24",
  width,
  strokeWidth = 0,
  onClick = () => {},
  size = 20,
}: IconProps) => {
  return (
    <svg
      stroke={strokeColor || fill || color}
      fill={fill}
      strokeWidth={strokeWidth?.toString()}
      viewBox={viewBox}
      className={className}
      height={height?.toString() || size?.toLocaleString()}
      width={width?.toString() || size?.toLocaleString()}
      xmlns="http://www.w3.org/2000/svg"
      onClick={onClick}
    >
      <path fill="none" d="M0 0h24v24H0z"></path>
      <path d="M12 9c1.65 0 3 1.35 3 3s-1.35 3-3 3-3-1.35-3-3 1.35-3 3-3m0-2c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0a.996.996 0 000-1.41l-1.06-1.06zm1.06-10.96a.996.996 0 000-1.41.996.996 0 00-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36a.996.996 0 000-1.41.996.996 0 00-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z"></path>
    </svg>
  );
};

const LayoutToggler = ({
  className = "",
  color,
  fill = "currentColor",
  height,
  strokeColor = "currentColor",
  viewBox = "0 0 24 24",
  width,
  strokeWidth = 0,
  onClick = () => {},
  size = 1.3,
  strokeLinecap,
  strokeLinejoin,
  role,
}: IconProps) => {
  return (
    <div className="rotate-90 d-flex justify-content-between align-items-center">
      <svg
        stroke={strokeColor || fill}
        fill={fill}
        strokeWidth={strokeWidth?.toString()}
        viewBox={viewBox}
        color={color}
        className={`cursor-pointer ${className ? className : ""}`}
        height={`${height ? height : size}em`}
        width={`${width ? width : size}em`}
        xmlns="http://www.w3.org/2000/svg"
        onClick={onClick}
        role={role}
        strokeLinecap={strokeLinecap}
        strokeLinejoin={strokeLinejoin}
        style={{ color: color ? color : "" }}
      >
        <path d="M4 9l4-4 4 4H9v6H7V9H4z" />
        <path d="M20 15l-4 4-4-4h3V9h2v6h3z" />{" "}
      </svg>
    </div>
  );
};

export { Moon, Sun, LayoutToggler };
