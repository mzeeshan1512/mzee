...
const ParticleAnimation= ({ lineCount=30 }:ParticleAnimationProps) => {
  const layoutThemeContext = useContext(LayoutThemeContext);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const ctxRef:any = useRef<CanvasRenderingContext2D | null>(null);
  const particles: Particle[] = [];
  const FPS = +process?.env?.NEXT_PUBLIC_PARTICLE_ANIMATION_FRAMES!?.toString() || 10;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        ctx.fillStyle = layoutThemeContext?.themeState?.mode === "dark" ? "white" : "black";
        ctx.beginPath();
      ...
      ctx.lineWidth = 0.05;
      ctx.strokeStyle =layoutThemeContext?.themeState?.mode === "dark" ? "white" : "black";
      ctx.stroke();
    };
  ...
  }, [lineCount, layoutThemeContext?.themeState]);
  return (
    <canvas
      ref={canvasRef}
